...
/**
 * FRAPI model for the SSD block. Renders the {@linkplain SSDBlockEntity#getCamo() camo} block's
 * geometry on all faces, then overlays the seven-segment display (an off/blank frame or a lit,
 * emissive digit) on the block's {@link HorizontalDirectionalBlock#FACING} side.
 * <p>
 * A single stateless instance serves every block state: facing/lit/value are read from the
 * {@code state} passed to {@link #emitQuads}, and the camo from per-block render data.
 */
public final class CamoDisplayModel implements BlockStateModel, BlockStateModel.UnbakedRoot {
    ...
    /**
     * Distance the overlay sits proud of the face, to win the depth test against the camo face.
     */
    ...
    private static TextureAtlasSprite displaySprite(final BlockState state) {
        if(state.getValue(SSDBlock.LIT)) {
            return sprite("block/digit_" + state.getValue(SSDBlock.VALUE));
        }
    ...
    public Object visualEqualityGroup(final BlockState state) {
        // One dynamic model backs every state; treat them as a single bake group.
        return this;
    }
    ...
    private void emitCamo(
        ...
        final Object data = ((FabricBlockView)blockView).getBlockEntityRenderData(pos);
        ...
        for(final BlockModelPart part : camoModel.collectParts(random)) {
            ((FabricBlockModelPart)part).emitQuads(emitter, cullTest);
        }
    }

    private void emitOverlay(final QuadEmitter emitter, final BlockState state) {
        ...
        emitter.spriteBake(displaySprite(state), MutableQuadView.BAKE_LOCK_UV);
