...
Rubric does what YetAnotherConfigLib ([YACL](https://modrinth.com/mod/yacl)) originally set out to do: be a *configuration library* — and nothing more. It defines your config,
loads and saves it, validates and migrates it, and syncs it between server and client. It does not draw the settings screen itself.
...
installed. Today that means [YACL](https://modrinth.com/mod/yacl) and [Cloth Config](https://modrinth.com/mod/cloth-config), with more frontends possible via the `ScreenProvider`
SPI. If neither is installed, Rubric still loads and edits your config on
disk — the in-game screen falls back to a placeholder that points the user at the config file.

## Scope

- Loader-agnostic core, with Fabric and NeoForge integration modules.
