...
[`../adapter-common/`](../adapter-common) and is shared with the YACL adapter. This module only contains the Cloth / MC-typed `ClothScreenProvider`, the loader-specific
`ClothConfigProviderFactory`, and the NG mod entry classes; the sibling [`../cloth-adapter-fabric/`](../cloth-adapter-fabric) module ships the same behavior on Fabric, and
[`../yacl-adapter-neoforge/`](../yacl-adapter-neoforge) is the YACL-backed sibling on NG.
...
For end users: install `coal.jar` (NeoForge variant), `coal-cloth-adapter-neoforge.jar`, and Cloth Config side-by-side in the mods folder. Consumer mods that call
`Coal.register(...)` automatically pick up this provider (priority `100` beats bundled `coal-noop`'s `0`).
...
Identical to the Fabric variant — the persistence + schema layer all lives in `coal-adapter-common` (shared with the YACL adapter), and this module supplies the Cloth-specific
screen rendering.
...
Feature-set gaps relative to the COAL spec + Cloth Config's own capability set. Everything here also applies to the Fabric variant — the underlying code is shared.
